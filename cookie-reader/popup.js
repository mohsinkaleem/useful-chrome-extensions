...
async function init() {
  ...
  // Set up download txt button
  document.getElementById('download-txt-btn').addEventListener('click', downloadCookiesAsTxt);
...
async function downloadCookiesAsTxt() {
  try {
    const cookies = await chrome.cookies.getAll({ url: currentUrl });
    if (!cookies || cookies.length === 0) {
    ...
    content += `# This file was generated by Cookie Manager\n`;
    content += `# Domain: ${currentDomain}\n`;
    ...
    cookies.forEach(c => {
      ...
      content += `${c.domain}\t${includeSubdomains}\t${c.path}\t${secure}\t${expiration}\t${c.name}\t${c.value}\n`;
    });
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${currentDomain}-cookies.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  } catch (error) {
    console.error('Error downloading cookies as txt:', error);
    alert('Failed to download cookies as text file.');
  }
}