...
<div
  class="rounded-lg border shadow-sm transition-colors {variantClasses[variant] ||
    variantClasses.default}"
>
  <!-- Header -->
  <!-- svelte-ignore a11y-no-noninteractive-tabindex -->
  <div
    class="px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 flex items-center justify-between {collapsible
  ...
  >
    <div class="flex items-center gap-2">
      ...
      <div>
        <h3
          class="text-sm font-semibold {headerVariantClasses[variant] ||
            headerVariantClasses.default}"
        >
          {title}
        </h3>
        {#if subtitle}
    ...
    <div class="flex items-center gap-2">
      ...
      {#if collapsible}
        <span
          class="text-gray-400 dark:text-gray-500 transform transition-transform {collapsed
            ? ''
            : 'rotate-180'}"
        >
          ▼
