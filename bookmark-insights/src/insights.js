...
export async function getDeadLinkInsights() {
  try {
    ...
    const byAge = {
      ...
      moderate: deadLinks.filter((b) => b.dateAdded <= oneWeekAgo && b.dateAdded > oneMonthAgo)
    ...
    const checkedCount = bookmarks.filter(
      (b) => b.isAlive !== null && b.isAlive !== undefined,
    ).length;
    const deadLinkRate =
      checkedCount > 0 ? ((deadLinks.length / checkedCount) * 100).toFixed(1) : 0;
...
export async function getContentAnalysis() {
  try {
    const bookmarks = await getBookmarksCached();
    const pct = (count) =>
      bookmarks.length > 0 ? Math.round((count / bookmarks.length) * 1000) / 10 : 0;
...
export async function getDomainIntelligence() {
  try {
    ...
    const dependencyWarnings = Object.values(domainData)
      .filter((d) => d.total / total >= 0.1)
      .map((d) => ({
        domain: d.domain,
        count: d.total,
        percentage: Math.round((d.total / total) * 100),
        topCategory:
          Object.entries(d.categories).sort(([, a], [, b]) => b - a)[0]?.[0] || 'unknown',
      }))
    ...
    const diversityScore =
      total > 0 ? Math.min(100, Math.round((uniqueDomains / total) * 100 * 3)) : 0;
    ...
    const knowledgeMap = Object.values(domainData)
      .map((d) => ({
        ...
        topCategory: Object.entries(d.categories).sort(([, a], [, b]) => b - a)[0]?.[0] || null,
...
export async function getTimeBasedAnalysis() {
  try {
    ...
    const peakHours =
      maxHourCount === 0
        ? []
        : bookmarkingHours.filter((h) => h.count === maxHourCount).map((h) => h.hourLabel);
    ...
    const percentOfTotal = (count) =>
      bookmarks.length > 0 ? Math.round((count / bookmarks.length) * 100) : 0;
  ...
  try {
    ...
    const leaderboard = Object.values(creatorMap)
      .map((c) => ({
        ...
        velocity:
          c.firstSaved !== Infinity
            ? (
                c.totalCount / Math.max(1, (now - c.firstSaved) / (30 * 24 * 60 * 60 * 1000))
              ).toFixed(1)
            : 0,
      }))
