<script>
  ...
  onMount(async () => {
    ...
    try {
      const [allBooks] = await Promise.all([loadBookmarks(), loadReadingList()]);
...
<div
  class="w-full h-screen overflow-hidden flex flex-col bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors"
>
  <!-- Header -->
  <div
    class="flex-shrink-0 p-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800"
  >
  ...
  <!-- Navigation Tabs -->
  <div
    class="flex-shrink-0 flex border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"
  >
    <button
      class="flex-1 px-2 py-2 text-xs font-medium transition-colors {viewMode === 'all'
    ...
    <button
      class="flex-1 px-2 py-2 text-xs font-medium transition-colors flex items-center justify-center gap-1 {viewMode ===
      'reading-list'
        ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400'
        : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}"
    ...
    >
      ...
      {#if unreadCount > 0}
        <span
          class="bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 text-[10px] px-1.5 py-0.5 rounded-full"
          >{unreadCount}</span
        >
      {/if}
    </button>
    <button
      class="flex-1 px-2 py-2 text-xs font-medium transition-colors {viewMode === 'quick-access'
        ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400'
        : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}"
    ...
    <button
      class="flex-1 px-2 py-2 text-xs font-medium transition-colors {viewMode === 'browse'
        ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400'
        : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'}"
  ...
  {#if currentFilter}
    <div
      class="flex-shrink-0 px-3 py-2 bg-blue-50 dark:bg-blue-900/30 border-b border-blue-100 dark:border-blue-800"
    >
  ...
  {#if undoAction}
    <div
      class="flex-shrink-0 px-3 py-2 bg-gray-800 dark:bg-gray-700 text-white flex items-center justify-between"
    >
      <span class="text-xs">Item removed from reading list</span>
      <button on:click={undoRemove} class="text-xs text-blue-400 hover:text-blue-300 font-medium">
        Undo
      </button>
    </div>
  {/if}
  ...
  <div class="flex-1 overflow-y-auto">
        ...
        {#if readingList.length === 0}
          <div class="text-center py-8">
            ...
            <p class="text-xs text-gray-400 dark:text-gray-500">
              Add pages to your reading list from Chrome's context menu
            </p>
          </div>
        {:else}
          <!-- Unread Section -->
          {#if readingList.filter((r) => !r.hasBeenRead).length > 0}
            ...
            >
              Unread ({readingList.filter((r) => !r.hasBeenRead).length})
            </h3>
            <div class="space-y-1 mb-4">
              {#each readingList.filter((r) => !r.hasBeenRead) as item}
                <div
                  class="group flex items-start gap-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                >
                  <img
          ...
          {#if readingList.filter((r) => r.hasBeenRead).length > 0}
            <h3
              class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2"
            >
              Read ({readingList.filter((r) => r.hasBeenRead).length})
            </h3>
            <div class="space-y-1">
              {#each readingList.filter((r) => r.hasBeenRead) as item}
                <div
                  class="group flex items-start gap-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors opacity-60"
                >
                  <img
                    src={getFaviconUrl({ url: item.url, domain: item.domain, faviconUrl: null })}
                    alt=""
    ...
    {:else if viewMode === 'quick-access'}
      ...
      <div class="p-3 space-y-4">
        ...
        {#if recentBookmarks.length > 0}
          <div>
            <h3
              class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 flex items-center gap-1"
            >
              🕐 Recently Added
            </h3>
            <div class="space-y-1">
              {#each recentBookmarks.slice(0, 8) as bookmark}
                ...
                >
                  <img src={getFaviconUrl(bookmark)} alt="" class="w-4 h-4 flex-shrink-0 rounded" />
        ...
        {#if recentBookmarks.length === 0 && frequentlyAccessed.length === 0}
          <div class="text-center py-8">
            ...
            <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">
              Recently added and frequently used bookmarks will appear here
            </p>
          </div>
        {/if}
      </div>
    {:else if viewMode === 'browse'}
      ...
      >
        <button
          class="flex-1 px-3 py-1.5 text-xs transition-colors {browseMode === 'recent'
        ...
        <button
          class="flex-1 px-3 py-1.5 text-xs transition-colors {browseMode === 'domains'
            ? 'text-blue-600 dark:text-blue-400 bg-white dark:bg-gray-800'
            : 'text-gray-500 dark:text-gray-400'}"
        ...
        <button
          class="flex-1 px-3 py-1.5 text-xs transition-colors {browseMode === 'folders'
            ? 'text-blue-600 dark:text-blue-400 bg-white dark:bg-gray-800'
            : 'text-gray-500 dark:text-gray-400'}"
      ...
      {#if browseMode === 'recent'}
        <div class="p-3 space-y-1">
          ...
          {#each [{ days: 1, label: 'Today' }, { days: 7, label: 'Last 7 days' }, { days: 14, label: 'Last 2 weeks' }, { days: 30, label: 'Last month' }, { days: 90, label: 'Last 3 months' }] as filter}
            <button
