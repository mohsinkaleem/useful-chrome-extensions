...
<div
  class="bookmark-card bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow cursor-pointer border border-gray-200 dark:border-gray-700 relative group {deadLinkClasses} {selectedClasses}"
  role="button"
  tabindex="0"
  ...
  <div class="flex items-start space-x-3">
    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="flex items-center gap-1 flex-shrink-0">
          ...
          {#if bookmark.accessCount > 0}
            <span
              ...
              title="Accessed {bookmark.accessCount} times"
            >
              <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div class="flex items-center justify-between mt-2 gap-2 flex-wrap">
        <div class="flex items-center gap-1.5">
          <span
            class="text-xs text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/40 px-2 py-1 rounded"
          >
            {getDomainLabel(bookmark)}
          </span>
          {#if bookmark.category}
            <span
              class="text-xs text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/40 px-2 py-0.5 rounded"
              title="Category: {bookmark.category}"
            >
              🏷️ {bookmark.category}
            </span>
          {/if}
        ...
        {#if bookmark.description}
          <p
            class="text-xs text-gray-500 dark:text-gray-400 mt-2 line-clamp-2 w-full"
        ...
        {#if bookmark.readingTime || bookmark.publishedDate || bookmark.contentQualityScore}
          <div class="flex items-center gap-2 mt-2 flex-wrap">
            {#if bookmark.readingTime}
              <span
                class="text-xs text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 px-2 py-0.5 rounded flex items-center gap-1"
            ...
            {#if bookmark.contentQualityScore}
              {@const qualityClass =
                bookmark.contentQualityScore >= 70
              ...
              <span
                class="text-xs px-2 py-0.5 rounded flex items-center gap-1 {qualityClass}"
        ...
        {#if bookmark.smartTags && bookmark.smartTags.length > 0}
          <div class="flex items-center gap-1 mt-2 flex-wrap">
            {#each bookmark.smartTags.slice(0, 5) as tag}
              <span
                class="text-xs text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/40 px-1.5 py-0.5 rounded"
                title="Smart tag"
              >
                {tag}
              </span>
            ...
            {#if bookmark.smartTags.length > 5}
              <span class="text-xs text-gray-500 dark:text-gray-400"
                >+{bookmark.smartTags.length - 5} more</span
              >
            {/if}
          </div>
        {/if}
        {#if bookmark.topics && bookmark.topics.length > 0}
          <div class="flex items-center gap-1 mt-2 flex-wrap">
            {#each bookmark.topics.slice(0, 4) as topic}
              <span
                class="text-xs text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/40 px-1.5 py-0.5 rounded"
                title="Topic: {topic}"
              >
                🏷️ {topic}
              </span>
            ...
            {#if bookmark.topics.length > 4}
              <span class="text-xs text-gray-500 dark:text-gray-400"
                >+{bookmark.topics.length - 4} more</span
              >
            {/if}
          </div>
        {/if}
